...
if __name__ == "__main__" :
	fs = [files for root, dirs, files in os.walk('./')]
	f = open("1.txt", "a+")
	for fname in fs[0] :
		if fname != 'get_dataset.py' and fname != '1.txt' :
			print(fname)
			pe_dataset = PE_DATASET(fname)
			f.write(str(pe_dataset.get_num_of_suspicious_import_func()) + "," + str(pe_dataset.has_embeded_pefile()) + "\n")
	f.close()